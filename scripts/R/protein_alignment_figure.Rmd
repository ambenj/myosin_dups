...
```
```{r}
# Perform hypergeometric test for enrichment changes in C1, C2, and C3
MF_aa %>% 
  group_by(Domain) %>% 
  summarise(MF_by_domain = n(), .groups = "drop") %>% 
  mutate(MF_total = sum(MF_by_domain),
...
# Calculate dN/dS
```{r}
# Read codon-aligned sequences
alignment <- read.alignment(file = "/Volumes/lab_kingsley/ambenj/myosin_dups/analysis/protein_msa/All_CDS_w3outgroups_translation_alignment.fasta", format = "fasta", forceToLower = FALSE)
...
for (i in 1:(n - 1)) {
  for (j in (i + 1):n) {
    aln_pair <- list(
      nb = 2,
      nam = alignment_all$nam[c(i, j)],
      seq = alignment_all$seq[c(i, j)],
      com = ""
    )
    class(aln_pair) <- "alignment"
    
    result <- kaks(aln_pair)
    ka <- result$ka
    ks <- result$ks
    ka_ks <- if (ks == 0) NA else ka / ks
    
    results <- rbind(results, data.frame(
      seq1 = alignment_all$nam[i],
      seq2 = alignment_all$nam[j],
      ka = ka,
      ks = ks,
      ka_ks = ka_ks
    ))
...
```{r}
results %>% 
  mutate(ka_ks = if_else(ka_ks > 0, ka_ks, NA)) %>% 
  arrange(-ka_ks) %>% 
  write_tsv("/Volumes/lab_kingsley/ambenj/myosin_dups/analysis/protein_msa/All_CDS_w3outgroups_translation_alignment_kaks.txt")
```

```{r}
results %>% 
  filter(str_detect(seq1, "BEPA|RABS"), str_detect(seq2, "BEPA|RABS")) %>% 
  mutate(ka_ks = if_else(ka_ks > 0, ka_ks, NA)) %>% 
  arrange(-ka_ks)
```
