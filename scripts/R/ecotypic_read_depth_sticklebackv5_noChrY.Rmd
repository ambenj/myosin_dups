...
fig_table <- ga5_roi_cov %>% 
  ...
  mutate(Figure2B_label = case_when(samp == "BIGR_1_32_2007_03" ~ "BIGR_M",
                                samp == "BIGR_52_54_2008_02" ~ "BIGR_F",
                                samp == "LITC_0_05_2008_841" ~ "LITC_M",
                                samp == "LITC_23_32_2008_306" ~ "LITC_F",
                                samp == "MIDF_BLUP_2011_01" ~ "MIDF_M",
                                ...
                                TRUE ~ acronym),
         analysis_group = case_when(!is.na(Figure2B_label) & !is.na(`used _river_comparisons`) ~ "Global comparison, Hybrid zone",
                                    !is.na(Figure2B_label) ~ "Global comparison",
  ...
  select(samp, Figure2B_label, pop, GPS_north, GPS_east, ecotype, water_type, region, wg_norm_depth, analysis_group)

# Write table to output file
colnames(fig_table) <- c("Sample", "Figure2B label", "Population", "GPS (North)", "GPS(East)", "Ecotype", "Water type", "Ocean basin", "MYH3C3 normalized depth", "Analysis group(s)")
