...
#                                  variant_type == "other" ~ "non-informative",
                                  TRUE ~ variant_type),
...
```{r}
# Since CMCB, DNSE, KFSY, and JADE results are very similar, filter down to one representative
div_pos_plot_all(filter(all_nogaps_df, sample %in% c("BEPA", "ECHO", "CMCB", "BLAU"))) + 
  facet_grid(rows = vars(sample), scale="free_y", space="free_y", switch = "y") +
