...
depth_plot <- roi_cov %>% 
  filter(desc == "MYH3C3") %>% 
  ggplot(aes(yrs_since_found, chrXIX_norm_depth, color = pop)) +
  geom_point(data = filter(roi_cov, desc == "MYH3C3", pop == "RABS"), size = 2.5, shape=5) +
  geom_line(data = filter(roi_cov, desc == "MYH3C3", pop != "RABS"),size = 1) +
  geom_point(data = filter(roi_cov, desc == "MYH3C3", pop != "RABS"), size = 1.5) +
    theme_cowplot(8) +
  panel_border(color="black", size=0.75) +
  theme(legend.position = c(0.8, 0.3)) +
  scale_color_manual("Population", values=c("#d55e00", "#cc79a7", "#7a669b", "#56b4e9")) +
...
af_plot
  ...
  geom_point(data = filter(af, pos == 2744769, pop == "RABS"), size = 2.5, shape=5) +
...
```
```{r}
final_plot <- plot_grid(depth_plot, af_plot, ncol = 1, rel_heights = c(1, 1), labels = c("B","C"), label_size = 12)
final_plot
```
