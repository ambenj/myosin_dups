...
# --- USER INPUT ---
ref_name <- "BEPA-C3a"
input_file <- "/Volumes/lab_kingsley/ambenj/myosin_dups/analysis/protein_msa/all_protein_FSmod_mafft_aligndivergent_sites.tsv"
...
domain_df <- read_tsv(domains_file) %>% 
  mutate(TileColor = case_when(
    Domain == "SH3" ~ "#a6cee3",
    Domain == "Myosin motor" ~ "#1f78b4",
    Domain == "IQ motif" ~ "#b2df8a",
    Domain == "Myosin tail" ~ "#33a02c",
    TRUE ~ NA_character_
  ))
...
#    DisplayAminoAcid = ifelse(Sequence == ref_name | IsDifferent, AminoAcid, ""),
    ...
    TileCategory == "stop" ~ "red",
    TileCategory == "diff" ~ "yellow",
    TileCategory == "FS" ~ "purple",
    is.na(TileCategory) ~ "grey",
    TRUE ~ TileColor
  ))
...
# Heatmap plot
heatmap_plot <- ggplot(long_df, aes(x = factor(Position), y = reorder(New_GroupLabel, Order), fill = TileColor)) +
  geom_tile(color = "gray90") +
  geom_text(aes(label = DisplayAminoAcid), na.rm = TRUE, size=1.8) +
...
ggsave(out_file, full_plot, width = 17.4, height = 6, units = "cm")
